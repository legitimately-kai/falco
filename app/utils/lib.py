import fnmatch, time, re, glob, os, imp, datetime
...
def isOp(irc, user):
    ...
    for ops in irc.ops:
        if fnmatch.fnmatch(user.hostmask, ops):
            return True
    try:
        return True if user.nick in irc.channels["#opers"]["nicks"] else False
    except:
        return False
...
class Command:
    def __init__(self, args):
        self.ircmsg = args.split(' ')
        self.cmdprefix = self.ircmsg[0][0:1]
        self.cmd = self.ircmsg[0][1:]
        self.options = args.split(cmd+' ')[1]
